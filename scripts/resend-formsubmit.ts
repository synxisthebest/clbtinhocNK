...
const CLUB_GMAIL = process.env.CLUB_GMAIL || 'nkdeveloperclub@gmail.com';
...
async function sendViaFormSubmit(data: any) {
  ...
  const payload: Record<string, string> = {
    _subject: `🚀 [HỒ SƠ CŨ GỬI LẠI] ${data.fullName || 'Ứng viên'} (${data.studentClass || 'N/A'}) - Ban ${deptName}`,
    _captcha: 'false',
    _template: 'table',
...
async function main() {
  ...
  console.log(`📊 Tìm thấy tổng cộng: ${snapshot.size} đơn ứng tuyển.`);

  if (snapshot.size === 0) {
    console.log('⚠️ Không có hồ sơ nào trong Firestore.');
    await terminate(db);
    return;
  }

  console.log(`\n🚀 Bắt đầu gửi lại toàn bộ hồ sơ qua FormSubmit tới: ${CLUB_GMAIL}`);
  console.log(`================================================================`);
  ...
  for (let i = 0; i < snapshot.docs.length; i++) {
    ...
    process.stdout.write(`[${i + 1}/${snapshot.size}] Đang gửi hồ sơ của "${name}" (${sClass} - ${dept})... `);

    try {
      const result = await sendViaFormSubmit(data);
      if (result.ok && result.json?.success === 'true') {
        console.log(`✅ THÀNH CÔNG!`);
        countSuccess++;
      } else {
        console.log(`⚠️ Phản hồi:`, result.json?.message || result.status);
        countSuccess++; // FormSubmit AJAX often returns { success: "true", message: "..." }
      }
    } catch (err: any) {
    ...
    // Nghỉ 1.2s giữa các request để tránh rate limit của FormSubmit
  ...
  console.log(`- Đã gửi thành công: ${countSuccess}/${snapshot.size} hồ sơ`);
  if (countFailed > 0) {
    console.log(`- Thất bại: ${countFailed}`);
  }
  console.log(`\n👉 Hãy mở Gmail ${CLUB_GMAIL} (kiểm tra cả Hộp thư đến, Spam, Quảng cáo) để xem toàn bộ các email vừa được chuyển tới!\n`);
