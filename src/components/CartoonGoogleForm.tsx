...
export function CartoonGoogleForm({ initialDepartment, onSuccessSubmitted }: CartoonGoogleFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Chi tiết câu hỏi và câu trả lời bài thi Casting theo từng Ban (Đặt tên chuẩn không chứa ký tự ngoặc vuông)
      if (formData.castingCpQ1) fsPayload.append('Cau 1 (Chuyen Tin - CP) Quan sat: Co gi sai? (2-4-8-16-31-64)', formData.castingCpQ1);
      if (formData.castingCpQ2) fsPayload.append('Cau 2 (Chuyen Tin - CP) Suy luan: Thong tin nao dang tin?', formData.castingCpQ2);
      if (formData.castingCpQ3) fsPayload.append('Cau 3 (Chuyen Tin - CP) Tu duy linh hoat: Doi luat 21 que', formData.castingCpQ3);
      if (formData.castingCpQ4) fsPayload.append('Cau 4 (Chuyen Tin - CP) Tu duy toi uu: Lam it hon', formData.castingCpQ4);
      if (formData.castingCpQ5) fsPayload.append('Cau 5 (Chuyen Tin - CP) Cau phan loai: Ban se hoi gi?', formData.castingCpQ5);

      if (formData.castingDevQ1) fsPayload.append('Cau 1 (Dev Web/App) Van de Web/App giai quyet', formData.castingDevQ1);
      if (formData.castingDevQ2) fsPayload.append('Cau 2 (Dev Web/App) Feature thich nhung user khong can', formData.castingDevQ2);
      if (formData.castingDevQ3) fsPayload.append('Cau 3 (Dev Web/App) Nut bam co the pha ca san pham', formData.castingDevQ3);
      if (formData.castingDevQ4) fsPayload.append('Cau 4 (Dev Web/App) Nguoi dung dang noi doi ban?', formData.castingDevQ4);
      if (formData.castingDevQ5) fsPayload.append('Cau 5 (Dev Web/App) Neu chi duoc giu lai mot thu', formData.castingDevQ5);

      if (formData.castingGameQ1) fsPayload.append('Cau 1 (Game Dev) Tai sao ban khong choi nua?', formData.castingGameQ1);
      if (formData.castingGameQ2) fsPayload.append('Cau 2 (Game Dev) Skill vs Luck', formData.castingGameQ2);
      if (formData.castingGameQ3) fsPayload.append('Cau 3 (Game Dev) Mechanic dong bang 3 giay', formData.castingGameQ3);
      if (formData.castingGameQ4) fsPayload.append('Cau 4 (Game Dev) Nguoi choi lam dieu ngoai y muon', formData.castingGameQ4);
      if (formData.castingGameQ5) fsPayload.append('Cau 5 (Game Dev) Core Loop 5 phut dau tien', formData.castingGameQ5);

      if (formData.castingAiResQ1) fsPayload.append('Cau 1 (AI Research) Du lieu hay mat minh (Robot A vs B)', formData.castingAiResQ1);
      if (formData.castingAiResQ2) fsPayload.append('Cau 2 (AI Research) Thu nghiem 9/10 lan', formData.castingAiResQ2);
      if (formData.castingAiResQ3) fsPayload.append('Cau 3 (AI Research) Con so noi doi (95% vs 90%)', formData.castingAiResQ3);
      if (formData.castingAiResQ4) fsPayload.append('Cau 4 (AI Research) Robot hoc dieu khong day', formData.castingAiResQ4);
      if (formData.castingAiResQ5) fsPayload.append('Cau 5 (AI Research) Chung minh minh sai', formData.castingAiResQ5);

      if (formData.castingHrQ1) fsPayload.append('Cau 1 (HR Nhan Su) Dung voi ket luan ve mot nguoi', formData.castingHrQ1);
      if (formData.castingHrQ2) fsPayload.append('Cau 2 (HR Nhan Su) Mot cau chuyen, 2 su that', formData.castingHrQ2);
      if (formData.castingHrQ3) fsPayload.append('Cau 3 (HR Nhan Su) Ung vien hoan hao va Dinh kien', formData.castingHrQ3);
      if (formData.castingHrQ4) fsPayload.append('Cau 4 (HR Nhan Su) Cong bang khong co nghia la giong nhau', formData.castingHrQ4);
      if (formData.castingHrQ5) fsPayload.append('Cau 5 (HR Nhan Su) Khi nao nen noi Khong', formData.castingHrQ5);

      if (formData.castingContentQ1) fsPayload.append('Cau 1 (Content) Mot chuyen - 3 goc nhin', formData.castingContentQ1);
      if (formData.castingContentQ2) fsPayload.append('Cau 2 (Content) Mot tinh huong - Mot meme', formData.castingContentQ2);
      if (formData.castingContentQ3) fsPayload.append('Cau 3 (Content) Quan ly Page trong 1 tuan', formData.castingContentQ3);

      if (formData.castingDesignQ1) fsPayload.append('Cau 1 (Design) Dung lam poster chi dep', formData.castingDesignQ1);
      if (formData.castingDesignQ2) fsPayload.append('Cau 2 (Design) Video 10 giay khong noi', formData.castingDesignQ2);
      if (formData.castingDesignQ3) fsPayload.append('Cau 3 (Design) Y tuong dau tien bi loai', formData.castingDesignQ3);

      if (formData.castingCamQ1) fsPayload.append('Cau 1 (Media) 5 tam anh ke 1 ngay', formData.castingCamQ1);
      if (formData.castingCamQ2) fsPayload.append('Cau 2 (Media) Bien dieu thuong thanh dang nho', formData.castingCamQ2);
      if (formData.castingCamQ3) fsPayload.append('Cau 3 (Media) 1 tam anh duy nhat', formData.castingCamQ3);
